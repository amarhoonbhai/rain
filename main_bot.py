...
# Contact shown in .upgrade docs (should match worker_forward.py)
UPGRADE_CONTACT = os.getenv("UPGRADE_CONTACT", "@SpinifyAdsBot or your reseller")
...
def _cmds_text():
    return (
      "📜 Self-Commands (send from your <b>logged-in account</b>, not this bot)\n\n"
      "Basic (free plan):\n"
      ...
      "• <code>.time &lt;value&gt;[m|h]</code> – full custom interval (e.g. <code>.time 10</code>, <code>.time 90</code>, <code>.time 2h</code>)\n"
      "• <code>.delay &lt;sec&gt;</code> – custom per-message delay between forwards\n"
      "• <code>.night</code> / <code>.night on</code> / <code>.night off</code> / <code>.night 23:00-07:00</code> – Auto-Night quiet hours\n\n"
      f"To upgrade, use <code>.upgrade</code> from your own account and send your ID to {UPGRADE_CONTACT}."
    )
...
async def cb_acc(cq: CallbackQuery):
    ...
    try:
        await cq.message.edit_text("👤 Manage Accounts\n" + "\n".join(lines), reply_markup=kb)
    except TelegramBadRequest:
        pass
...
async def fstats(msg: Message):
    ...
    await msg.answer(
        ...
        f"Interval (panel): {interval} min\n"
        f"Sessions: {ss} | Groups: {gs}\n"
        f"Next send (approx): {eta}\n"
...
async def prem_up(cq: CallbackQuery, state:FSMContext):
    ...
    await cq.message.edit_text(
        "Send <code>user_id</code> to upgrade:",
        reply_markup=InlineKeyboardMarkup(
            inline_keyboard=[[InlineKeyboardButton(text="⬅ Back", callback_data="menu:home")]]
        )
    )
...
async def prem_up_id(msg: Message, state:FSMContext):
    ...
    await state.set_state(OwnerFSM.prem_name)
...
async def prem_down(cq: CallbackQuery, state:FSMContext):
    ...
    await cq.message.edit_text(
        "Send <code>user_id</code> to downgrade:",
        reply_markup=InlineKeyboardMarkup(
            inline_keyboard=[[InlineKeyboardButton(text="⬅ Back", callback_data="menu:home")]]
        )
    )
...
async def prem_down_id(msg: Message, state:FSMContext):
    ...
    await state.clear()
    await msg.answer(f"✅ Premium disabled for {target} (cap=5).")
