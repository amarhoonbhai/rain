# core/db.py
# SQLite store for Spinify (users, sessions, groups, ads, settings, stats)
...
def upsert_session_slot(user_id: int, slot: Optional[int], api_id: int, api_hash: str, session_string: str) -> int:
    """Insert/update a session at given slot; if slot is None, pick first free."""
    if slot is None:
    ...
    """, (user_id, slot, api_id, api_hash, session_string, now))
    conn.commit()
    conn.close()
    return slot

# Back-compat single-slot helper (slot=1)
...
def get_gate_channels_effective() -> Tuple[Optional[str], Optional[str]]:
    """Returns (ch1, ch2); caller applies defaults if both None/empty."""
...
# (optional getters if your enforcer uses them)
def get_name_lock(user_id: int) -> Tuple[bool, Optional[str]]:
    en = str(get_setting(f"name_lock:{user_id}:enabled", "0")).lower() in ("1", "true")
